...
export class ControllerProfileStore {
  ...
  async load() {
    try {
      ...
      const profile = normalizeControllerProfile(JSON.parse(serialized));
      this.#lastLoadError = null;
      return profile;
    } catch (error) {
      this.#lastLoadError = error;
      return createDefaultControllerProfile();
    }
  }

  async save(profile) {
    const normalized = normalizeControllerProfile(profile);
...
export function defaultControllerProfilePath({
