...
export function defaultCodexKeybindingsPath() {
  return join(homedir(), ".codex", "keybindings.json");
}

export async function installVibePocketCodexKeybindings(
  filePath = defaultCodexKeybindingsPath(),
) {
  const existing = await readKeybindings(filePath);
  assertNoConflicts(existing);

  const managedCommands = new Set(VIBE_POCKET_CODEX_KEYBINDINGS.map(({ command }) => command));
  const bindings = existing.filter(({ command, key }) => !managedCommands.has(command) || key !== null);
  for (const binding of VIBE_POCKET_CODEX_KEYBINDINGS) {
  ...
  return {
    filePath,
    installed: VIBE_POCKET_CODEX_KEYBINDINGS.length,
    bindings,
  };
}
