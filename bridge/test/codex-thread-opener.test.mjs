...
import { openCodexThread } from "../src/codex-thread-opener.mjs";
...
test("opens an exact Codex thread through each platform URI handler", async () => {
  ...
  for (const [platform, command, args] of cases) {
    ...
    assert.deepEqual(await openCodexThread("019f-test", { platform, spawnProcess: recorded.spawnProcess }), {
...
test("rejects unsafe thread IDs before launching a URI handler", async () => {
  ...
  await assert.rejects(
    () => openCodexThread("thread/../../settings", { spawnProcess: recorded.spawnProcess }),
    /invalid thread ID/i,
  );
