...
import {
  installVibePocketCodexKeybindings,
  VIBE_POCKET_CODEX_KEYBINDINGS,
} from "../src/codex-keybindings.mjs";
...
test("installs fixed desktop semantic shortcuts without replacing user bindings", async () => {
  ...
  await installVibePocketCodexKeybindings(filePath);
  await installVibePocketCodexKeybindings(filePath);
...
test("refuses to silently steal a shortcut from another command", async () => {
  ...
  const filePath = join(directory, "keybindings.json");
  await writeFile(filePath, JSON.stringify([
    { command: "some.user.command", key: VIBE_POCKET_CODEX_KEYBINDINGS[0].key },
  ]));

  await assert.rejects(
    () => installVibePocketCodexKeybindings(filePath),
    /already assigned/,
  );
});
