...
import { RolloutActivityReader } from "../src/rollout-activity-reader.mjs";
...
test("derives and incrementally updates running state from Codex rollout lifecycle markers", async (t) => {
  ...
  const reader = new RolloutActivityReader({ sessionsRoot, chunkSize: 64 });
...
test("ignores rollout paths outside the Codex sessions root", async (t) => {
  ...
  const reader = new RolloutActivityReader({ sessionsRoot });
