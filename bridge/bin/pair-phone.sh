...
LOCAL_PROTOCOL=$(/usr/bin/curl -fsS "http://127.0.0.1:$VIBE_POCKET_PORT/healthz" | \
...
REMOTE_PROTOCOL=$(/usr/bin/curl -fsS "$PUBLIC_URL/healthz" | \
  $NODE_BIN -e 'let body=""; process.stdin.on("data", c => body += c); process.stdin.on("end", () => process.stdout.write(String(JSON.parse(body).protocolVersion)))')
...
RESPONSE=$(/usr/bin/curl -fsS \
  --unix-socket "$PAIRING_SOCKET" \
  -X POST \
  -H "Content-Type: application/json" \
  --data "$PAYLOAD" \
  "http://localhost/v1/pairing/invitations")
PAIRING_URL=$($NODE_BIN -e 'process.stdout.write(JSON.parse(process.argv[1]).pairingUrl)' "$RESPONSE")

ADB_BIN=${VIBE_POCKET_ADB:-$(command -v adb 2>/dev/null || true)}
if [[ -n "$ADB_BIN" ]]; then
  DEVICES=(${(f)"$($ADB_BIN devices | awk 'NR > 1 && $2 == "device" { print $1 }')"})
  if (( ${#DEVICES} == 1 )); then
    if $ADB_BIN -s "$DEVICES[1]" shell am start -W \
      -a android.intent.action.VIEW \
      -d "'$PAIRING_URL'" \
      -p au.edu.uts.vibepocket >/dev/null 2>&1; then
...
if ! /usr/bin/open -n -a "$HOST_APP" --args show-pairing-file "$PAIRING_FILE"; then
  rm -f "$PAIRING_FILE"
